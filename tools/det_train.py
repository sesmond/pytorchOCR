...
def ModelTrain(train_data_loader, t_model, t_criterion, model, criterion, optimizer, loss_bin, args, config, epoch):
    ...
    for batch_idx, data in enumerate(train_data_loader):
        ...
        if (config['base']['algorithm'] == 'DB'):
            ...
            iou, acc = cal_DB(pre_batch['f_score'], gt_batch['input_score'], gt_batch['input_mask'],
                              running_metric_text)
        else:
            iou, acc = cal_PAN_PSE(pre_batch['pre_kernel'], gt_batch['gt_kernel'], pre_batch['pre_text'],
                                   gt_batch['gt_text'],
    ...
    for batch_idx, (imgs, ori_imgs) in enumerate(test_data_loader):
        ...
        for i in range(img_num):
            if (config['base']['algorithm'] == 'SAST'):
                scale = ((out['f_score'].shape[2] * 4) / ori_imgs[i].shape[0],
                         (out['f_score'].shape[3] * 4) / ori_imgs[i].shape[1], ori_imgs[i].shape[0],
                         ori_imgs[i].shape[1])
            else:
...
def TrainValProgram(args):
    ...
    if args.pruned_model_dict_path is not None:
        ...
        title.extend(
            ['piexl_acc', 'piexl_iou', 't_rescall', 't_precision', 't_hmean', 'b_rescall', 'b_precision', 'b_hmean'])
    ...
    for epoch in range(start_epoch, config['base']['n_epoch']):
        ...
        loss_write = ModelTrain(train_data_loader, t_model, distil_loss, model, criterion, optimizer, loss_bin, args,
                                config, epoch)
